...
name=name.trim();
if(name.length()==0) throw new DAOException("Length is zero");
...
{
if(employeeId==null) throw new DAOException("Employee od is null");
if(employeeId.length()==0) throw new DAOException("Length of employee id is zero.");
try
{
File file=new File(FILE_NAME);
...
{
randomAccessFile.close();
throw new DAOException("Invlaid employee id. : "+employeeId);
}
randomAccessFile.readLine();
randomAccessFile.readLine();
EmployeeDTOInterface employeeDTO;
